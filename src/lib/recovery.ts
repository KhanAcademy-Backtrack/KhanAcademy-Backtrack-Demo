...
export type Event = { kind: 'khan_open' | 'khan_feedback' | 'practice_report' | 'other_support' | 'pause' | 'resume' | 'next_turn'; skill: Skill; at: number; detail?: string };
...
export function recoveryReducer(prev: Recovery, action: RecoveryAction): Recovery {
  ...
  if (action.type==='resume') {const phase=s.next==='pause'?'check':s.next;return {...s, phase, active:s.nextSkill,serial:phase==='check'?nextFreshSerial(s,s.serial,s.nextSkill):s.serial, blockCount:0, events:[...s.events,{kind:'resume',skill:s.nextSkill,at:action.now}]};}
  ...
  const ev:Attempt={id:p.id,skill:s.active,answer:action.answers,correct,assisted:s.assisted,confidence:action.confidence,at:action.now,purpose:s.returnCheck?'return':s.extension?'next':'route',problemVersion:s.problemVersion===2?2:1};
  ...
  if (!correct) {
    ...
    if(clue&&misses<3&&!s.evidence.some(e=>e.skill!=='goal'&&Number(e.id.split(':').at(-1))>=(s.cycleStart??0))){
...
export function validRecovery(value: unknown): value is Recovery {
  ...
  return s.version===1 && Object.keys(TOPICS).includes(s.topic) && ['self','class'].includes(s.mode) && phases.has(s.phase) && phases.has(s.next) && skills.has(s.active) && skills.has(s.nextSkill)
    && (s.problemVersion===undefined||s.problemVersion===2) && (s.exposedPairs===undefined||(Array.isArray(s.exposedPairs)&&s.exposedPairs.length<=2000&&s.exposedPairs.every(x=>typeof x==='string'&&/^-?\d+(?:\.\d+)?,-?\d+(?:\.\d+)?$/.test(x))))
