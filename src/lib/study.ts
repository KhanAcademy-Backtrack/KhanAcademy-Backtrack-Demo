...
import {initialRecovery,problemFor,nextFreshSerial,pairKey,TOPICS,ORDER,validRecovery,recentSuccesses,answerInputIssue,type Topic,type Skill,type Recovery,type Attempt} from './recovery.ts';
...
export type KhanReturn={topic:Topic;skill:Skill;title:string;url:string;returnPath:string;at:number;feedback?:KhanFeedback};
...
export type StudyEvent={kind:'challenge_open'|'challenge_attempt'|'pack_shared'|'pack_saved'|'khan_open'|'khan_report'|'khan_feedback'|'round_done'|'session_done';at:number;detail:string};
...
export function recordKhanOpen(s:StudyState,activity:KhanReturn,routeDetail?:string){return {...s,pendingKhan:activity,updatedAt:activity.at,seenEvents:routeDetail?[...s.seenEvents,`${activity.topic}:khan_open:${activity.skill}:${activity.at}:${routeDetail}`]:s.seenEvents,events:[...s.events,{kind:'khan_open' as const,at:activity.at,detail:`${activity.topic}:${activity.skill}:${activity.title}`}].slice(-1000)};}
export function recordKhanFeedback(s:StudyState,activity:KhanReturn,feedback:KhanFeedback,now:number):StudyState{
  ...
  return {...s,review:{...s.review,[key]:item},pendingKhan:{...activity,feedback},seenEvents:[...s.seenEvents,`${activity.topic}:khan_feedback:${activity.skill}:${now}:${feedback}`],events:[...s.events,{kind:'khan_feedback' as const,at:now,detail:`${key}:${feedback}`}].slice(-1000),updatedAt:now};
}
...
export function prepareRound(s:StudyState,task:StudyTask,now:number):Recovery{
  ...
  const base:Recovery={...old,problemVersion:2,evidence:old.evidence.map(e=>({...e,problemVersion:e.problemVersion??(old.problemVersion===2?2:1)})),mode:'self',budget:60,blockLimit:10,blockCount:0,phase:task.mode==='learn'?'learn':'check',active:task.skill,next:'check',nextSkill:task.skill,planned:[...new Set([...TOPICS[task.topic].path,task.skill])].sort((a,b)=>ORDER.indexOf(a)-ORDER.indexOf(b)),passed:old.passed.filter(x=>x!==task.skill),goalPassed:task.skill==='goal'?false:old.goalPassed,extension:false,returnCheck:task.mode==='review'&&task.skill==='goal'&&!!s.review[skillKey(task.topic,task.skill)]?.independentAt,assisted:false,correct:null,startedAt:old.startedAt||now,updatedAt:now,sharedSerialFloor:s.cursors[task.topic]??0,exposedPairs:[...new Set([...(old.exposedPairs??[]),...(s.pairs[task.topic]??[])])],cycleSkill:task.skill};
