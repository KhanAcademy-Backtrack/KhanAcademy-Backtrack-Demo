import {validScope,type ScopeEntry} from './scope-coverage.ts';
...
import {validExplore,isExploreReturnPath,type ExploreState} from './explore.ts';
...
export type ReviewItem={key:string;topic:Topic;skill:Skill;dueAt:number;stage:number;streak:number;lastAt:number;lastDifficulty?:number;lastAssisted?:number;independentAt?:number;manual:boolean;paused:boolean;khanFeedback?:KhanFeedback;khanFeedbackAt?:number;scheduledAt?:number;retainedAt?:number};
...
export type StudySession={id:string;packId:string;minutes:number;tasks:StudyTask[];deferred:number;startedAt:number;lastAt:number;index:number;complete:boolean;rewarded:boolean;difficultyAdjusted:boolean;meaningful?:boolean;xpEarned?:number;note?:string;rehearsal?:boolean;oneQuestionPerTask?:boolean};
...
export type StudyState={version:1;updatedAt:number;activePack:string;packs:Pack[];review:Record<string,ReviewItem>;seen:string[];seenEvents:string[];routes:Partial<Record<Topic,Recovery>>;cursors:Partial<Record<Topic,number>>;pairs:Partial<Record<Topic,string[]>>;sessions:StudySession[];activeSession?:StudySession;xp:number;awards:string[];lastVisit:number;settings:{tourDone?:boolean;minutes:number;weeklyGoal:number;quiet:boolean;sound:boolean;style:'leaf'|'sky'|'peach';accessory:'none'|'leaf'|'star'|'sun';intervals:[number,number,number]};events:StudyEvent[];pendingKhan?:KhanReturn;explore?:ExploreState};
export const initialStudy=():StudyState=>({version:1,updatedAt:0,activePack:'',packs:[],review:{},seen:[],seenEvents:[],routes:{},cursors:{},pairs:{},sessions:[],xp:0,awards:[],lastVisit:0,settings:{minutes:10,weeklyGoal:3,quiet:false,sound:false,style:'leaf',accessory:'none',intervals:[1,7,14]},events:[]});
...
const fingerprint=(s:Recovery,e:Attempt)=>`${skillKey(s.topic,e.skill)}:${e.expression??problemFor({...s,problemVersion:e.problemVersion===1?undefined:e.problemVersion===2||e.problemVersion===3?e.problemVersion:s.problemVersion,active:e.skill,serial:Number(e.id.split(':').at(-1))}).expression}`;
export function ingestRecovery(state:StudyState,route:Recovery,now:number):StudyState{
  ...
  for(const e of route.evidence){
    ...
    const p=problemFor({...route,problemVersion:e.problemVersion===1?undefined:e.problemVersion===2||e.problemVersion===3?e.problemVersion:route.problemVersion,active:e.skill,serial:Number(e.id.split(':').at(-1))});if(p.factorPair)pairs.add(pairKey(p.factorPair));
    ...
    else{item.streak++;if(old?.independentAt&&e.at-old.independentAt>=DAY)item.retainedAt=e.at;item.independentAt=e.at;if(item.streak%2===0&&(!item.scheduledAt||e.at-item.scheduledAt>=DAY)){item.dueAt=e.at+state.settings.intervals[Math.min(item.stage,2)]*DAY;item.stage=Math.min(item.stage+1,2);item.scheduledAt=e.at;}}
    review[id]=item;
  }
...
export function planRehearsal(s:StudyState,pack:Pack,minutes:number,now:number):StudySession{const base=planSession(s,pack,minutes,'challenge',now);return {...base,rehearsal:true,oneQuestionPerTask:true,tasks:[...base.tasks,...base.tasks.map(t=>({...t,id:t.id+'-return'}))]};}
...
export function prepareRound(s:StudyState,task:StudyTask,now:number):Recovery{
  ...
  const base:Recovery={...old,probes:0,draft:undefined,original:undefined,problemVersion:3,evidence:old.evidence.map(e=>({...e,problemVersion:e.problemVersion??(old.problemVersion??1)})),mode:'self',budget:60,blockLimit:10,blockCount:0,phase:task.mode==='learn'?'learn':'check',active:task.skill,next:'check',nextSkill:task.skill,planned:[...new Set([...TOPICS[task.topic].path,task.skill])].sort((a,b)=>ORDER.indexOf(a)-ORDER.indexOf(b)),passed:old.passed.filter(x=>x!==task.skill),goalPassed:false,extension:false,returnCheck:task.mode==='review'&&!!s.review[skillKey(task.topic,task.skill)]?.independentAt,assisted:false,correct:null,startedAt:old.startedAt||now,updatedAt:now,sharedSerialFloor:s.cursors[task.topic]??0,exposedPairs:[...new Set([...(old.exposedPairs??[]),...(s.pairs[task.topic]??[])])],cycleSkill:task.skill};
...
export function completeStudyTask(s:StudyState,route:Recovery,now:number,taskId:string):StudyState{
  ...
  if(new Set(recorded.map(e=>e.id)).size<(active.oneQuestionPerTask?1:2))return s;
  const unique=new Set<string>();
  const answered=route.evidence.filter(e=>{if(e.at<(active.oneQuestionPerTask?active.startedAt:task.startedAt??active.startedAt)||e.at>now)return false;const p=problemFor({...route,problemVersion:e.problemVersion===1?undefined:e.problemVersion===2||e.problemVersion===3?e.problemVersion:route.problemVersion,active:e.skill,serial:Number(e.id.split(':').at(-1))});const key=fingerprint(route,e);if(answerInputIssue(p,e.answer)||unique.has(key))return false;unique.add(key);return true;});
...
export function parseSharedPack(params:URLSearchParams):Pack|undefined{
  let scope:ScopeEntry[]|undefined;const coverage=params.get('coverage');if(coverage){if(coverage.length>16000)return;try{const parsed=JSON.parse(coverage);if(!validScope(parsed))return;scope=parsed.map(({text,topic,referenceOnly})=>({text,topic,referenceOnly}));}catch{return;}}const curriculum=params.get('curriculum')?.slice(0,120);
  const raw=params.get('scope');if(!raw)return;const topics=raw.split('.');if(!topics.length||topics.length>9||topics.some(x=>!Object.hasOwn(TOPICS,x)))return;
  ...
  const name=(params.get('title')??'Shared study pack').trim().slice(0,60)||'Shared study pack';let hash=2166136261;for(const char of unique.join('.')+':'+name+':'+JSON.stringify(scope??[])+':'+(curriculum??''))hash=Math.imul(hash^char.charCodeAt(0),16777619)>>>0;return {id:origin?`khan-${origin.id}`:`shared-${hash.toString(36)}`,name,description:'Shared scope. Your progress and support stay personal.',topics:unique,custom:true,khanId:origin?.id,scope,curriculum};
}
export function validStudy(x:unknown):x is StudyState{
  ...
  const pack=(p:Pack)=>obj(p)&&text(p.id)&&text(p.name,60)&&text(p.description,1000)&&Array.isArray(p.topics)&&p.topics.length>0&&p.topics.length<=9&&p.topics.every(t=>Object.hasOwn(TOPICS,t))&&(p.khanId===undefined||!!khanEntry(p.khanId))&&(p.cards===undefined||Array.isArray(p.cards)&&p.cards.length<=40&&p.cards.every(validStudyCard))&&(p.scope===undefined||validScope(p.scope))&&(p.curriculum===undefined||text(p.curriculum,120))&&(p.testDate===undefined||p.testDate===''||/^\d{4}-\d{2}-\d{2}$/.test(p.testDate));
