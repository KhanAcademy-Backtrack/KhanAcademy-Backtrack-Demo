...
export function replayModel(s:Recovery):ReplayModel|undefined{
  ...
  const candidate=[...s.evidence].reverse().find(e=>!e.correct&&e.answer.length===2&&e.answer.every(x=>/^[+-]?(?:\d+(?:\.\d*)?|\.\d+)$/.test(x.replace(/[−–－]/g,'-')))&&
    (e.skill===s.active||e.skill==='goal'));
  if(candidate){
    const p=problemFor({...s,problemVersion:candidate.problemVersion===1?undefined:candidate.problemVersion===2?2:s.problemVersion,active:candidate.skill as Skill,serial:Number(candidate.id.split(':').at(-1))});
    if(p.factorPair)return {kind,pair:p.factorPair,attempt:candidate.answer.map(x=>Number(x.replace(/[−–－]/g,'-'))) as [number,number],fromAttempt:true};
  }
  const p=problemFor(s);
  return {kind,pair:p.factorPair??[3,4],attempt:kind==='factors'?[2,6]:[3,4],fromAttempt:false};
}
