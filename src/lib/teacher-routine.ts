...
export function importRouteRecord(raw:string):Recovery{
  ...
  if(!validRecovery(data))throw new Error('This is not a supported Khanpanion route record.');
  return data;
}
...
export function actionSheetCsv(rows:TeacherRow[],illustrative:boolean,week?:number){
  const head=['Learner code','Teaching week','Class goal','Next action','Khanpanion checks','Khan assignment evidence','Next return date','Record type'];
