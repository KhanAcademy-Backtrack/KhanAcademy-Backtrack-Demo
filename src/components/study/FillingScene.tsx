...
export function FillingScene({autoplay=false,compact=false,staticMode=false,saved,onChange}:{autoplay?:boolean;compact?:boolean;staticMode?:boolean;saved?:{rate?:number;start?:number;time?:number};onChange?:(values:{rate:number;start:number;time:number})=>void}){
 ...
 useEffect(()=>{playback.current?.stop();if(!running||off||!visible)return;if(time.get()>=5)time.set(0);const p=animate(time,5,{duration:(5-time.get())*1.5,ease:'linear',onComplete:()=>{setRunning(false);onChange?.({rate:flow.get(),start:start.get(),time:5});}});playback.current=p;return()=>p.stop();},[running,off,visible,time]);
 ...
 return <div ref={stage} className={`filling-scene ${compact?'scene-compact':''}`} data-running={running&&!off&&visible}>
  ...
  {(rate!==2||initial!==6)&&<p className="scene-reference">Dashed line: the original 6 L start and 2 L/min rate.</p>}<p className="scene-insight">{reading.time===5&&!running?'Five minutes shown. Replay or try another rate.':rate===2?'The water and the point rise together.':'The line turns about the same starting value.'} <strong>Move the dot or the time slider.</strong></p>
