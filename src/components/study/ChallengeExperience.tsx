...
import {packFromKhan,beginSession,planSession,skillKey,type StudyEvent} from '@/lib/study';
...
function ChallengeRound({challenge:c}:{challenge:Challenge}){
 ...
 const logged=useRef(false),entry=khanEntry(c.khanId)!,pack=packFromKhan(entry),correct=checked&&!unknown&&isCorrect(c.question,values),nextStep=challengeNextStep(confidence,correct,unknown);
 ...
 return <div className="study-space challenge-page"><div className="challenge-heading"><div><p className="eyebrow">Challenge {c.code}</p><h1>{c.title}</h1><p>{c.hook}</p></div><Link className="button-text" href="/packs">My packs ↗</Link></div>
  <section className="source-challenge"><h2>{c.question.prompt}</h2><MathText size="hero">{c.question.expression}</MathText>
   <form onSubmit={e=>{e.preventDefault();respond();}}><div className="answer-inputs">{c.question.labels.map((label,i)=><label className="number-field" key={label}><span>{label}</span><input value={values[i]} onChange={e=>{setValues(xs=>xs.map((x,j)=>i===j?e.target.value:x));setChecked(false);setUnknown(false);setError('');setExplain(false);}} maxLength={18} autoComplete="off" inputMode="text"/></label>)}</div>
  ...
  <section className="challenge-next"><h2>Keep going with this Khan activity.</h2><p>{entry.title}. The same learning goal continues into your pack and reviewer.</p><PackKhanResources pack={pack}/><button className="button-text" onClick={()=>{update(s=>({...s,packs:[...s.packs.filter(x=>x.id!==pack.id),pack],activePack:pack.id,updatedAt:Date.now()}));router.push('/packs');}}>Save this study pack ↗</button></section>
 </div>;
}
