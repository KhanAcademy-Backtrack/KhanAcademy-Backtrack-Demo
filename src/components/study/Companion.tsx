...
function Bookmark({accessory='none',size=120,pose='idle'}:{accessory?:'none'|'leaf'|'star'|'sun';size?:number;pose?:BuddyPose}){
 const reduced=useReducedMotion(),{state}=useStudy(),[gaze,setGaze]=useState({x:0,y:0}),[hello,setHello]=useState(false);
 const [typing,setTyping]=useState(false);useEffect(()=>{const sync=()=>setTyping(!!document.activeElement?.matches('input,textarea,select'));document.addEventListener('focusin',sync);document.addEventListener('focusout',sync);return()=>{document.removeEventListener('focusin',sync);document.removeEventListener('focusout',sync);};},[]);useEffect(()=>{if(!hello)return;const timer=setTimeout(()=>setHello(false),1300);return()=>clearTimeout(timer);},[hello]);
 const off=!!reduced||state.settings.quiet||typing,face=FACES[hello?'wave':pose];
 ...
 return <motion.svg className="study-companion expressive-buddy" data-pose={hello?'wave':pose} width={size} height={size} viewBox="0 0 128 124" role="img" aria-label={`Dunlo’s bookmark study buddy, ${hello?'waving hello':pose}`} onPointerMove={e=>{if(off)return;const r=e.currentTarget.getBoundingClientRect();setGaze({x:Math.max(-3,Math.min(3,(e.clientX-r.left-r.width/2)/r.width*8)),y:Math.max(-2,Math.min(2,(e.clientY-r.top-r.height/2)/r.height*5))});}} onPointerLeave={()=>{setGaze({x:0,y:0});setHello(false);}} onPointerDown={()=>setHello(true)}>
  ...
  <motion.g style={{transformOrigin:'63px 85px'}} animate={{rotate:face.tilt,y:pose==='aha'?-4:0,scaleX:pose==='aha'?1.025:1,scaleY:pose==='thinking'?.97:1}} transition={transition}>
   <motion.g animate={off?{y:0}:{y:[0,-2,0]}} transition={{duration:4.5,repeat:Infinity,ease:'easeInOut'}}>
    {face.arms.split('M').filter(Boolean).map((part,i)=><motion.path key={i} className="buddy-arms" d={'M'+part} style={{transformOrigin:'94px 61px'}} animate={{d:'M'+part,rotate:!off&&i===1&&(hello||pose==='wave')?[0,-12,8,-10,0]:0}} transition={{d:transition,rotate:off?{duration:0}:{duration:1.1,ease:'easeInOut'}}} stroke="#14bf96" strokeWidth="7" fill="none" strokeLinecap="round"/>)}
    ...
    <motion.g animate={{x:gaze.x+(pose==='point'?2:0),y:gaze.y}} transition={transition}>
     {[50,76].map((cx,i)=><motion.g key={cx} style={{transformOrigin:`${cx}px 46px`}} animate={off?{scaleY:face.eyes}:{scaleY:[face.eyes,face.eyes,.08,face.eyes,face.eyes]}} transition={off?{duration:0}:{duration:4.6,repeat:Infinity,times:[0,.76,.79,.82,1],delay:i*.025}}><ellipse cx={cx} cy="46" rx="4.5" ry="6.5" fill="#0a2a66"/><circle cx={cx+1.2} cy="43.5" r="1.2" fill="white" opacity=".85"/></motion.g>)}
    </motion.g>
    <motion.path d={face.mouth} animate={{d:face.mouth}} transition={transition} stroke="#0a2a66" strokeWidth="3.3" fill={pose==='aha'?'#0a2a66':'none'} strokeLinecap="round"/>
