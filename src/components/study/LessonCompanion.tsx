...
export function LessonCompanion({kind,step,onPoint,onExample}:{kind:MathsKind;step:number;onPoint:()=>void;onExample:()=>void}){const {state,update}=useStudy(),[help,setHelp]=useState(false);const quiet=state.settings.quiet;return <div className="lesson-companion" data-help={help}>
 <button className="buddy-call" aria-label="Ask your bookmark companion for a pointer" aria-expanded={help} onClick={()=>{setHelp(!help);onPoint();}}><Companion size={76} accessory={state.settings.accessory}/></button>
