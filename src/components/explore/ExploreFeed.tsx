...
export function ExploreFeed(){
 ...
 return <div className="explore-world" ref={root}><ExploreTour onClose={endTour}/>
  <header className="explore-intro"><div><p className="eyebrow">Khanpanion Explore</p><h1>Explore something new.</h1><p>Play with an idea. Learn with Khan Academy. Try it yourself.</p></div><div className="explore-intro-links"><img src="/khan-academy.svg" alt="Khan Academy" width="160" height="30"/><Link href={state.activeSession&&!state.activeSession.complete?'/study/session':'/'}>{state.activeSession&&!state.activeSession.complete?'Resume your study session':'Go to Today'} ↗</Link></div></header>
  ...
  <p className="explore-attribution">Khanpanion is an independent project. Original Khanpanion activities are labelled separately from Khan Academy lessons. All Khan Academy content is available for free at <a href="https://www.khanacademy.org/" target="_blank" rel="noopener noreferrer">khanacademy.org</a>.</p>
 </div>;
}
...
function IdeaCard({item,number,total,active,still,onShare}:{item:ExploreItem;number:number;total:number;active:boolean;still:boolean;onShare:()=>void}){
 ...
 return <article id={`idea-${item.id}`} data-idea={item.id} data-active={active} className={`explore-card format-${item.format}`} aria-labelledby={`title-${item.id}`}>
  <div className="idea-topline"><span>{String(number).padStart(2,'0')} / {String(total).padStart(2,'0')} · {item.format==='khan'?'Khan Academy lesson':'Khanpanion interactive → Khan practice'}</span><div><button type="button" onClick={()=>update(s=>saveExplore(s,item,Date.now()))} aria-pressed={saved} aria-label={`${saved?'Unsave':'Save'} ${item.title}`}>{saved?'Saved ✓':'Save'}</button><button type="button" onClick={onShare} aria-label={`Share ${item.title}`}>Share ↗</button></div></div>
