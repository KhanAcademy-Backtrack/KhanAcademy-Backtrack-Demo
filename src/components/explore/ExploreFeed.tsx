...
function IdeaCard({item,number,total,active,still,onShare}:{item:ExploreItem;number:number;total:number;active:boolean;still:boolean;onShare:()=>void}){
 ...
 return <article id={`idea-${item.id}`} data-idea={item.id} data-active={active} className={`explore-card format-${item.format}`} aria-labelledby={`title-${item.id}`}>
  ...
  </div><div className="idea-thinking"><p className="eyebrow">{answered?'The idea behind it':'A quick thought'}</p><h3>{item.question}</h3><div className="idea-choices" role="group" aria-label={item.question}>{item.choices.map((label,i)=><button key={label} type="button" aria-pressed={choice===i} data-answer={answered&&i===item.answer?'yes':undefined} onClick={()=>update(s=>answerExplore(s,item,i,Date.now()))}><span>{String.fromCharCode(65+i)}</span>{label}</button>)}</div>{!answered?<button className="idea-show-why" onClick={()=>update(s=>answerExplore(s,item,-1,Date.now()))}>I don’t know yet · Show me why</button>:<div className="idea-explanation" role="status"><strong>{choice===item.answer?'That’s the relationship.':choice===-1?'Let’s make it clear.':'Here’s how it works.'}</strong><p>{item.explanation}</p></div>}</div></div>
