...
export function GraphBoard({graph,guide=false}:{graph:GraphData;guide?:boolean}){
  ...
  return <figure className="graph-board"><svg viewBox="0 0 380 296" role="img" aria-label={alt}>
    ...
    {graph.kind==='line'&&<><rect x={x(graph.x)-14} y={238} width={28} height={23} rx={5} fill="#e7e5ff"/><text x={x(graph.x)} y={253} textAnchor="middle" fill="#403cd9" fontWeight="700">{graph.x}</text><path d={`M${x(graph.x)} 225 v-20 m-5 6 5-6 5 6`} stroke="#5753fa" fill="none" strokeWidth={2}/></>}
    <path d="M42 22 V232 H343 M37 28 l5-6 5 6 M337 227 l6 5-6 5" fill="none" stroke="#172a45" strokeWidth="1.5"/><text x={185} y={283} textAnchor="middle">x · across</text><text x={12} y={16}>y · up</text>
    {graph.kind==='point'?<><circle cx={x(graph.x)} cy={y(graph.y??0)} r={11} fill="#e8e5ff"/><circle cx={x(graph.x)} cy={y(graph.y??0)} r={6} fill="#5753fa"/><text x={x(graph.x)+13} y={y(graph.y??0)-11} fill="#5753fa">P</text></>:<line x1={x(0)} y1={y(b)} x2={x(maxX)} y2={y(m*maxX+b)} stroke="#087e70" strokeWidth={3}/>}
    {guide&&<><path d={`M${x(graph.x)} 232 V${y(graph.kind==='point'?graph.y??0:m*graph.x+b)} H42`} fill="none" stroke="#5753fa" strokeWidth={2} strokeDasharray="5 4"/><circle cx={x(graph.x)} cy={y(graph.kind==='point'?graph.y??0:m*graph.x+b)} r={5} fill="#5753fa"/></>}
