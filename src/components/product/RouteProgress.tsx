...
export function RouteProgress({state:s}:{state:Recovery}){
 const total=new Set(s.planned).size;
 const passed=new Set(s.passed.filter(x=>s.planned.includes(x))).size;
 const checkingReturn=s.returnCheck&&['check','feedback'].includes(s.phase);
 const amount=checkingReturn?Math.min(2,recentSuccesses(s,s.destinationSkill??'goal')):s.goalPassed?total*2:Math.min(total*2,passed*2+(s.passed.includes(s.active)?0:Math.min(1,recentSuccesses(s,s.active))));
