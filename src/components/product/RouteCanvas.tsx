...
export function RouteCanvas({ state, compact = false }: { state: Recovery; compact?: boolean }) {
  ...
  const points=nodes.map((id,i)=>({id,x:id==='goal'?30:58,y:34+i*78}));
  ...
  const status=(id:Skill)=>state.passed.includes(id)?'Checked':id===state.active?(state.phase==='setup'?'First check':'You’re here'):id==='goal'?'Where you’re headed':state.suspected.includes(id)?'Needs practice':'Check if you need this';
  return <div className={`route-canvas ${compact?'compact':''}`}>
    ...
    <div className="route-roadmap">
      <svg viewBox={`0 0 94 ${Math.max(68,nodes.length*78)}`} preserveAspectRatio="none" aria-hidden="true" className="roadmap-line">
        <motion.path className={`route-stroke ${state.suspected.length&&!state.goalPassed?'detour':''}`} initial={false} animate={{d:path}} transition={{duration:reduced?0:.65,ease:[.22,1,.36,1]}}/>
