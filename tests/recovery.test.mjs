...
import { initialRecovery, problemFor, recoveryReducer as reduce, isCorrect, validRecovery, ORDER } from '../src/lib/recovery.ts';
let clock=100000;
...
test('all generated exercises accept mathematics, root order and Unicode minus',()=>{
  for(const topic of ['quadratics','brackets'])for(const active of ORDER)for(let serial=0;serial<100;serial++){
...
test('both destinations complete after a genuine deep-gap route',()=>{
  for(const topic of ['quadratics','brackets']){
...
test('fresh exercises have different expressions, and quadratic roots are distinct',()=>{for(const topic of ['quadratics','brackets'])for(const active of ORDER){const seen=new Set();for(let serial=0;serial<120;serial++){const p=problemFor({...initialRecovery(topic),active,serial});assert.ok(!seen.has(p.expression),p.id+' repeated '+p.expression);seen.add(p.expression);if(p.unordered)assert.equal(new Set(p.expected).size,p.expected.length);}}});
