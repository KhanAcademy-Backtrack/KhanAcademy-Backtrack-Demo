...
const dir='.refs/living-scenes';await fs.mkdir(dir,{recursive:true});
const server=spawn(process.execPath,['scripts/serve-static.mjs'],{env:{...process.env,BACKTRACK_PORT:'3053'},windowsHide:true,stdio:'pipe'});await new Promise((r,j)=>{server.stdout.once('data',r);server.once('error',j);});
const browser=await chromium.launch({channel:'chrome',headless:true}),results=[];
async function run(name,work,options={}){const context=await browser.newContext({viewport:{width:1280,height:900},recordVideo:{dir:`${dir}/video`,size:{width:1280,height:900}},...options}),page=await context.newPage(),errors=[];page.on('pageerror',e=>errors.push(e.message));page.setDefaultTimeout(12000);try{await work(page);assert.deepEqual(errors,[]);results.push({name,passed:true});console.log('PASS '+name);}catch(e){results.push({name,passed:false,error:e.message});await page.screenshot({path:`${dir}/${name.replace(/\W/g,'-')}-failure.png`,fullPage:true});console.log('FAIL '+name+': '+e.message);}finally{await context.close();const video=await page.video()?.path();if(video&&name==='visible synchronized motion')await fs.copyFile(video,`${dir}/dunlo-live-model.webm`);}}
async function open(page){await page.goto('http://127.0.0.1:3053/');if(await page.getByRole('button',{name:'Explore on my own'}).isVisible())await page.getByRole('button',{name:'Explore on my own'}).click();}
const geometry=page=>page.evaluate(()=>{const svg=document.querySelector('.filling-drawing'),point=document.querySelector('.flow-point').getBoundingClientRect(),water=document.querySelector('.flow-water').getBoundingClientRect(),inverse=svg.getScreenCTM().inverse(),p=new DOMPoint(point.x+point.width/2,point.y+point.height/2).matrixTransform(inverse),w=new DOMPoint(water.x,water.y).matrixTransform(inverse),bottom=new DOMPoint(water.x,water.bottom).matrixTransform(inverse);return {x:p.x,y:p.y,waterY:w.y,waterH:bottom.y-w.y};});
try{
 ...
 await run('water stops at the end and pause resumes from the same place',async page=>{await open(page);await page.locator('.filling-scene[data-running=true]').waitFor();await page.waitForFunction(()=>Number(document.querySelector('.flow-point').getAttribute('cx'))>120);await page.getByRole('button',{name:'Pause filling animation',exact:true}).click();const paused=await geometry(page);assert.equal(await page.locator('.flow-stream').count(),0);await page.getByRole('button',{name:'Play filling animation',exact:true}).click();await page.waitForFunction(x=>Number(document.querySelector('.flow-point').getAttribute('cx'))>x+15,paused.x);await page.waitForFunction(()=>document.querySelector('.filling-scene').dataset.running==='false'&&Number(document.querySelector('.flow-point').getAttribute('cx'))>367);assert.equal(await page.locator('.flow-stream').count(),0);const ended=await geometry(page);await page.waitForTimeout(900);assert.ok(Math.abs((await geometry(page)).x-ended.x)<.1);await page.locator('.welcome-world').screenshot({path:dir+'/water-stopped.png'});await page.getByRole('button',{name:'Replay filling animation',exact:true}).click();await page.locator('.filling-scene[data-running=true]').waitFor();await page.waitForFunction(()=>Number(document.querySelector('.flow-point').getAttribute('cx'))>65&&Number(document.querySelector('.flow-point').getAttribute('cx'))<140);assert.equal(await page.locator('.flow-stream').count(),1);await page.getByRole('button',{name:'Pause filling animation',exact:true}).click();assert.equal(await page.locator('.flow-stream').count(),0);});
